...
export default function CommentSection({ postId }: CommentSectionProps) {
  ...
  const fetchCurationData = async (id: string) => {
    try {
      ...
      const res = await fetch(
        `https://api.linkurator.store/api/v1/curation/${id}`
      );
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.linkurator.store/api/v1/curations/${postId}/comments`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  const handleEditSave = async (commentId: number) => {
    ...
    try {
      ...
      // API 호출로 댓글 수정
      const response = await fetch(
        `https://api.linkurator.store/api/v1/curations/${postId}/comments/${commentId}`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
  ...
  const handleAddReply = async (commentId: number) => {
    ...
    try {
      ...
      // API 호출로 답글 생성
      const response = await fetch(
        `https://api.linkurator.store/api/v1/curations/${postId}/comments/${commentId}/reply`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  const handleEditReplySave = async (commentId: number, replyId: number) => {
    ...
    try {
      ...
      // API 호출로 답글 수정
      const response = await fetch(
        `https://api.linkurator.store/api/v1/curations/${postId}/comments/${commentId}/reply/${replyId}`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
