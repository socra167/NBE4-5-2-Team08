    ...
    async (incrementView = true) => {
      ...
      try {
        ...
        const response = await fetch(
          `https://52.78.27.224.nip.io/api/v1/playlists/${params.id}${queryString}`,
          {
        ...
        if (sessionStorage.getItem("isLoggedIn") === "true") {
          try {
            const likeStatusResponse = await fetch(
              `https://52.78.27.224.nip.io/api/v1/playlists/${params.id}/like/status?_t=${timestamp}`,
              {
                credentials: "include",
                cache: "no-store",
                headers: {
        ...
        try {
          const recResponse = await fetch(
            `https://52.78.27.224.nip.io/api/v1/playlists/${params.id}/recommendation?_t=${timestamp}`,
            {
              ...(sessionStorage.getItem("isLoggedIn") === "true"
                ? { credentials: "include" }
                : {}),
